...
#
# This script defines all settings shared across the pipeline:
# directory paths, target states, study years, and the year-specific
# variable name mappings that handle BRFSS naming changes.
#
...
# These 6 US states serve as untreated controls in the DiD comparing against Canada (federal recreational legalization Oct 2018).
...
# --- Year-specific variable name mappings -------------------------------------
# BRFSS renames some variables across survey years. This function returns a
# named character vector mapping raw column names (in the .XPT file) to
# standardized names used throughout the analysis. See README files for details. 
#
# How it works:
#   names(config)  = raw column names as they appear in the .XPT for that year
#   values(config) = standardized column names we use in the analysis dataset
#
# Example: setNames("marijuana_days", "MARIJANA") creates the element
#   "MARIJANA" = "marijuana_days"
# meaning "find the column called MARIJANA and rename it to marijuana_days."
#
...
#
# Variable name changes documented here:
...
#   Income:         INCOME2 (2016-20, 8 cats) -> INCOME3 (2021-22, 11 cats)
...
get_year_config <- function(yr) {
  ...
  config <- c(
    ...
    # Will be recoded to 3 categories in 02_clean_append.R:
    #   1 = less than secondary (EDUCA 1-3)
    #   2 = secondary (EDUCA 4)
    #   3 = some or completed post-secondary (EDUCA 5-6)
    ...
    # ---- ONES KWASI ALREADY HAD 
    # ,setNames("veteran",       "VETERAN3")
    # ,setNames("pregnant",      "PREGNANT")
    # ,setNames("smoke100",      "SMOKE100")
  )
